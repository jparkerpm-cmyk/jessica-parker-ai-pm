...
After a job search run, a results summary table is the default output — one row per scored result, ordered by score, with salary signal visible across all results at once. Full reports render on demand with "show [company]". Nothing re-runs. Detail was already computed internally.
